...
with tab_address:
    st.markdown("Tapez une adresse. L'application détecte le département, affiche les vues géographiques, cherche l'historique probable DVF, les comparables et le DPE.")
    address_input = st.text_input("Adresse", placeholder="Ex : 12 rue de la Paix, Vincennes")
    radius_history = st.slider("Rayon pour l'historique probable et les comparables", 30, 500, 120, step=10, format="%d m")
    ...
    if st.button("Rechercher", type="primary") and address_input:
        ...
        if geo is None:
            st.error("Adresse non reconnue par l'API Adresse. Essayez une formulation plus simple.")
        else:
            ...
            st.session_state["adresse_dept_detecte"] = detected_dept
            ...
            if detected_dept != dept:
                st.warning(f"Cette adresse semble être dans le département {detected_dept}, alors que la barre latérale est sur {dept}. Sélectionnez {detected_dept} dans la barre latérale puis téléchargez/construisez sa référence si nécessaire.")

            if not reference_exists(detected_dept):
                st.info(f"Construisez la référence de prix pour le département {detected_dept} afin d'obtenir l'historique DVF et les comparables.")
            else:
                st.subheader("Historique probable des ventes du bien / de l'adresse")
                try:
                    history = core.find_property_history(detected_dept, address_input, geo["latitude"], geo["longitude"], radius_m=radius_history)
                    if history.empty:
                        st.caption("Aucune vente DVF trouvée à proximité immédiate.")
                    else:
                        st.dataframe(history, use_container_width=True)
                        st.caption("Historique probable : la DVF publique ne garantit pas toujours l'identification exacte du logement, surtout en copropriété. Vérifiez les lignes proches et l'adresse DVF.")
                except SystemExit as e:
                    st.warning(str(e))

                st.subheader("Ventes comparables à proximité")
                try:
                    comparables = core.find_comparables(detected_dept, geo["latitude"], geo["longitude"], radius_m=radius_history)
...
with tab_potentiel:
    st.markdown(
        "Analyse du foncier à partir d'une adresse déjà recherchée dans "
        "l'onglet **📍 Adresse + vues** : cadastre, zone PLU et risques "
        "naturels/technologiques."
    )
    lat = st.session_state.get("adresse_lat")
    lon = st.session_state.get("adresse_lon")

    if lat is None or lon is None:
        st.info(
            "Recherchez d'abord une adresse dans l'onglet '📍 Adresse + vues' "
        ...
        if st.button("Analyser le foncier", type="primary"):
            ...
            st.caption(
                ...
                "ni la lecture du règlement de zone complet (hauteur, emprise au "
                "sol maximale, reculs, coefficient de biotope...) ni un "
...
with tab_score:
    ...
    if active_ref is None:
        ...
        with col2:
            ...
            prix = st.number_input("Prix affiché (€)", min_value=1000.0, value=450_000.0, step=1000.0)
